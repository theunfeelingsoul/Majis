...
if (isset($_GET['frm_date']) && isset($_GET['to_date']) && isset($_GET['faci_status'])):
	...
	public function Header() {
		...
		public function ColoredTable($header,$data) {
			...
			foreach($data as $key => $row) {
				$this->Cell($w[0], 6, $row['faci_con']== 1? 'Working':'Not working', 'LR', 0, 'L', $fill);
				$this->Cell($w[1], 6, $row['region'], 'LR', 0, 'L', $fill);
				$this->Cell($w[2], 6, $row['lga'], 'LR', 0, 'L', $fill);
				$this->Cell($w[3], 6, $row['ward'], 'LR', 0, 'L', $fill);
				$this->Cell($w[4], 6, $row['faci_num'], 'LR', 0, 'L', $fill);
				$this->Cell($w[5], 6, $row['faci_name'], 'LR', 0, 'L', $fill);
	...
	$pdf->SetSubject('Status Report by date');
	$pdf->SetKeywords('Amina, Nasoor, Report, Status, date');
	...
	$pdf->Output('status_report.pdf', 'I');
